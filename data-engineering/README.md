# Data Engineering 学习笔记

数据工程学习线：管道、仓、调度。

## 学习顺序（当前计划）

1. [DE101](./01-de101.md) — 先搞懂 DE 在干什么 + SQL / Python / Docker / dbt / Airflow 基础
2. [Data Engineering Zoomcamp](./02-data-engineering-zoomcamp.md) — 主线实战（9 周 pipeline）
3. [Handbook](./03-data-engineer-handbook.md) — 当地图查漏补缺，不当主教材

对比与选型见 [comparison.md](./comparison.md)；跟课顺序见 [path.md](./path.md)。

## 资源一览

| 资源 | 链接 | 角色 |
|------|------|------|
| DE101 | https://de101.startdataengineering.com/ | 基础书 + 代码 |
| Zoomcamp | https://github.com/DataTalksClub/data-engineering-zoomcamp | 主线免费课 |
| Handbook | https://github.com/DataExpert-io/data-engineer-handbook | 导航 / 书单 |
| Cookbook（可选） | https://github.com/andkret/Cookbook | 补充阅读 |

## 进度

- [ ] DE101 — 打基础
- [ ] Zoomcamp — 跟完整 cohort 或按模块自学
- [ ] Handbook — 按需查阅
- [ ] 动手：完成一个端到端小项目（ingest → transform → warehouse → 调度）
